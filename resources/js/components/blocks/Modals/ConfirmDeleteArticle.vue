<template lang="pug">
  DivElement(classCss="modal__layout, modal__layout_tiny, modal__layout_confirm-delete-article" :class="classIsConfirmShown()" :data-article_id="0")
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      deleteArticle(e){
        e.preventDefault()
        let article_id = e.target.closest('.modal__layout_active').dataset.article_id
        let sendData = new FormData()
        sendData.append('article_id', article_id)
        this.sendRequest({
          method: 'POST',
      ...
      successDelete(result){
        if(result.data.code == 0){
          // console.log(this.appendUserToStore, false)
